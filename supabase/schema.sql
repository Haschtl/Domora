...
create table if not exists households (
  ...
  task_mode text not null default 'rotation' check (task_mode in ('rotation', 'time')),
  ...
  household_map_markers jsonb not null default '[]'::jsonb,
...
create or replace view household_members_with_profiles
...
alter table households add column if not exists household_map_markers jsonb not null default '[]'::jsonb;
...
alter table households
alter column household_map_markers set not null;
...
revoke execute on function join_household_by_invite(text) from anon;
...
revoke execute on function rate_task_completion(uuid, integer) from anon;
...
revoke execute on function resolve_one_off_task_claim(uuid, text, numeric) from anon;

revoke all on function settle_task_time_vacation_credits(uuid) from public;
grant execute on function settle_task_time_vacation_credits(uuid) to authenticated, service_role;
revoke execute on function settle_task_time_vacation_credits(uuid) from anon;

revoke all on function resolve_task_time_correction_proposals(uuid) from public;
grant execute on function resolve_task_time_correction_proposals(uuid) to authenticated, service_role;
revoke execute on function resolve_task_time_correction_proposals(uuid) from anon;
...
revoke all on function queue_push_test_job(uuid, text, jsonb, timestamptz) from public;
grant execute on function queue_push_test_job(uuid, text, jsonb, timestamptz) to authenticated, service_role;
revoke execute on function queue_push_test_job(uuid, text, jsonb, timestamptz) from anon;

revoke all on function get_push_test_jobs(uuid, integer) from public;
...
revoke all on function get_push_test_job_logs(uuid, uuid) from public;
grant execute on function get_push_test_job_logs(uuid, uuid) to authenticated, service_role;
revoke execute on function get_push_test_job_logs(uuid, uuid) from anon;
...
revoke execute on function run_all_households_data_maintenance(boolean) from anon, authenticated;
revoke execute on function queue_push_from_household_event() from public, anon, authenticated;
revoke execute on function queue_push_on_bucket_add() from public, anon, authenticated;
revoke execute on function queue_push_on_shopping_add() from public, anon, authenticated;
revoke execute on function queue_push_on_task_takeover() from public, anon, authenticated;
