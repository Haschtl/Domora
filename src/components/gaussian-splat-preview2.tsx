...
const inferSplatExtension = (fileName: string) => {
  ...
  return SPLAT_EXTENSIONS.includes(
    extension as (typeof SPLAT_EXTENSIONS)[number],
  )
    ? extension
    : null;
};

export const isGaussianSplatFileName = (fileName: string) =>
  inferSplatExtension(fileName) != null;
      ...
      try {
        ...
        if (!bounds.isEmpty()) {
          ...
          camera.position.copy(
            center
              .clone()
              .add(new THREE.Vector3(radius * 1.8, radius * 0.6, radius * 1.8)),
          );
          ...
          {status === "error" ? (
            <>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  {errorMessage ??
                    "Die Datei konnte nicht als Gaussian Splat geladen werden."}
                </p>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  Spark initialisiert Renderer und dekodiert die Splat-Datei.
                </p>
              </div>
            </>
          )}
